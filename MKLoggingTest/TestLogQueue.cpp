...
TEST(LogQueue, Basic)
{
    ...
    EXPECT_CALL(*mockSink, OutputString(testString))
        .RetiresOnSaturation();

    // Run the test
    bufferedSink->OutputString(testString);
...
TEST(LogQueue, MultiThreaded)
{
    ...
    CLogCentral logger({ bufferedLog, std::make_shared<CLogDebugOutputSink>() });
    //CLogCentral logger({ bufferedLog });
