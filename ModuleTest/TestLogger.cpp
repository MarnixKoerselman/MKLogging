...
import LogFileSink;
import LogQueue;
import LogDebugOutputSink;
import LogUnbufferedFileSink;
...
TEST(Logger, Utf8)
{
  ...
  std::string actualMessage = stringSink->Buffer.substr(stringSink->Buffer.rfind(testMessage.c_str()));
  ...
  std::u8string_view szOriginal = u8"Hello World\n你好世界";
  testMessage.assign(szOriginal.cbegin(), szOriginal.cend());
...
TEST(Logger, CombineStringTypes)
{
  ...
  auto helloWorldChinese = reinterpret_cast<const char*>(u8"\n\t你好世界");

  MKL_LOGI(&logger, "\n\tHello world" << L"\n\tHello world" << helloWorldChinese << helloWorldChinese << L"\n\t\x4f60\x597d\x4e16\x754c");
