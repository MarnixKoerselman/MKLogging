...
TEST(LogFormatters, DefaultLogFormatter)
{
  ...
  EXPECT_CALL(mockSink, OutputRecord(testing::_)).WillOnce([function, line](const LogRecord& record)
    {
...
TEST(LogFormatters, MessageOnlyLogFormatter_LogRecordWithSink)
{
  ...
  {
    LogRecordAutoSink(buffer.get(), ELogLevel::Info, __func__, __FILE__, __LINE__).Get() << "test";
  }
...
TEST(LogFormatters, DerivedLogFormatterPartials)
{
    ...
    {
    public:
        void OutputLogLevel(std::ostream& os, ELogLevel /*logLevel*/) override
        {
            os << "loglevel";
        }
        void OutputTime(std::ostream& os, const std::chrono::system_clock::time_point& /*time*/) override
        {
            os << "time";
        }
        void OutputThreadId(std::ostream& os, std::thread::id /*threadId*/) override
        {
            os << "threadid";
        }
        void OutputMessage(std::ostream& os, const LogRecord& /*record*/) override
        {
            os << "message";
        }
    };
