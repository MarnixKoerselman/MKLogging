...
void CLogUnbufferedFileSink::WriteToFile(const std::string& text)
{
	WriteToFile(text.data(), text.size() * sizeof(std::string::value_type));
}

void CLogUnbufferedFileSink::OutputRecord(const LogRecord& record)
{
...
void CLogUnbufferedFileSink::WriteToFile(const void* data, size_t size)
{
	if (m_FileDescriptor != 0)
	{
