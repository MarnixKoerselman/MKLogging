...
{
	// If there is a formatter registered, then prepare the preformatted message and store it in the record.
	...
	std::shared_ptr<LogRecord> recordWithPreformattedMessage;
	if (m_Formatter)
	{
		std::ostringstream buffer;
		OutputFormattedRecord(buffer, record);

		recordWithPreformattedMessage = std::make_shared<LogRecord>(record); // Make a temporary log record that contains the preformatted message. Don't change the 'source' record, that would mess with the responsibilities in the log chain.
